# --------------------------------------------------------
# HermiteFlow-VFI (FlowFormer-based variant)
#
# Identical to HermiteFlow_R but uses FlowFormer instead of RAFT
# as the optical flow backbone. Key differences:
# - No feature projection layers (FlowFormer outputs at target dims)
# - Uses flow_estimator.build_coord() for correlation lookup
# - FlowFormer returns flow as list, take [0]
#
# References:
...
from .modules.fi_utils import warp, resize


class HermiteFlow_F(nn.Module):
    """
    HermiteFlow-VFI with FlowFormer backbone.

    Same architecture as HermiteFlow_R but uses FlowFormer for flow estimation.
    FlowFormer outputs features at the target dimensions directly, so no
    projection layers are needed.
    """
    ...
    def __init__(self, config: HermiteFlowConfig):
        ...
        if getattr(config, "pretrained_decoder_ckpt", None) is None:
            config.pretrained_decoder_ckpt = "pretrained/gimmvfi_f_arb_lpips.pt"

        # ============== Module 1: The Head (FlowFormer) ==============
        self.flow_estimator = initialize_Flowformer(config.pretrained_flowformer_ckpt)
    ...
    def cal_bidirection_flow(self, im0, im1):
        """Module 1 — The Head: extract flows + features via FlowFormer."""
        ...
        # FlowFormer returns flow as a list
        f01 = f01[0]
        f10 = f10[0]
    ...
    def frame_synthesize(
        ...
        """Module 5 — The Tail: AMT-style decoder (identical to GIMM-VFI-F)."""
        batch_size = img_xs.shape[0]
        img0 = 2 * img_xs[:, :, 0] - 1.0
        img1 = 2 * img_xs[:, :, 1] - 1.0

        # FlowFormer uses its own build_coord
    ...
    def sample_coord_input(
        ...
        """API compatibility stub — HermiteFlow doesn't use INR coordinates."""
