...
class GIMMVFI_R(nn.Module):
    ...
    def __init__(self, config: GIMMVFIConfig):
        ...
        # Upstream calls initialize_RAFT() with no args, hardcoding
        # "pretrained_ckpt/raft-things.pth" - wired to the config field
        # instead so this reads the same --raft-ckpt this project's own
        # HermiteFlow_R uses (see configs.py's pretrained_raft_ckpt).
